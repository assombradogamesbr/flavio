...
module.exports = class Factory extends Command {
  ...
  async run({ message, args, prefix, author }, t) {
    ...
    if (["work", "trabalhar"].includes(args[0].toLowerCase())) {
      ...
      if (cooldown > 0) {
        ...
        message.channel.send(
          `${message.author}, você trabalhou com sucesso e conseguiu **${XP} XP** para sua Fábrica e **R$${COINS}** que já foram depositados no seu banco.`
        );
    ...
    if (["name", "nome"].includes(args[0].toLowerCase())) {
        ...
        if (name.length > 220)
          return message.reply(
            `${message.author}, o nome da fábrica deve conter 220 ou menos caracteres.`
          );
