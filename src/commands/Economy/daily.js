...
module.exports = class Daily extends Command {
  ...
  async run({ message, args, prefix, author }, t) {
    ...
    const give = Math.floor(Math.random() * 1000);
    let cooldown = 3600000;
    let coins = user.vip.hasVip ? give + Math.floor(Math.random() * 1500) : give;
    ...
    if (daily !== null && cooldown - (Date.now() - daily) > 0) {
      ...
      message.channel.send(
        `${
          message.author
        }, você resgatou seu prêmio diário de hoje e conseguiu **${coins}** sexo coins.\nAgora você possui **${Utils.toAbbrev(
          atual + coins
        )}** coins.`
      );
